	...
	class DatabaseAccessObject{
		...
		//MEMBER Table
			...
			********************************************************/
			...
			public function addMember($firstName,$lastName,$email,$password,$profession,$address,$city,$country,$photographURL,$dateOfBirth,$privacy){
				try{
					$hashedPassword = crypt($password, '$5$abcdefghijkl1234');
					$defaultPrivilege = "junior";
					$defaultStatus = "active";
					$statement = $this->db->prepare('INSERT INTO Member(firstName,lastName,email,hashedPassword,profession,address,city,country,photographURL,hearts,dateOfBirth,privacy,privilege,status)
					VALUES(:firstName, :lastName, :email, :hashedPassword, :profession, :address, :city, :country, :photographURL, :hearts, :dateOfBirth, :privacy, :defaultPrivilege, :defaultStatus);');
			...
			public function getMemberInfo($memberId){
				try{
					$statement = $this->db->prepare('SELECT memberId, firstName, lastName, email, profession, address, city, country, photographURL, hearts,
					dateOfBirth, privacy, privilege, status FROM Member WHERE memberId = :memberId;');
		...
		// WALL CONTENT Table
			...
			public function postWallContent($memberId, $permissionId, $previousPoster, $originalPoster, $contentType, $content){
				$newValue = 1;
				try{
				...
				try{
					$statement = $this->db->prepare('INSERT INTO WallContent(memberId, wallContentNumber, permissionId, previousPoster, originalPoster, contentType, content)
					VALUES(:memberId, :wallContentNumber, :permissionId, :previousPoster, :originalPoster, :contentType, :content);');
					$statement->bindValue(':memberId', $memberId, PDO::PARAM_INT);
					$statement->bindValue(':wallContentNumber', $newValue, PDO::PARAM_INT);
					$statement->bindValue(':permissionId', $permissionId, PDO::PARAM_INT);
					$statement->bindValue(':previousPoster', $previousPoster, PDO::PARAM_INT);
					$statement->bindValue(':originalPoster', $originalPoster, PDO::PARAM_INT);
			...
			public function getWallContentInfo($memberId, $wallContentNumber){
				try{
					$statement = $this->db->prepare('SELECT memberId, wallContentNumber, permissionId, previousPoster, originalPoster, contentType, content, timeStamp
		...
		// GROUPCONTENT Table
			...
			public function postGroupContent($groupId, $memberId, $permissionId, $wrote, $previousPoster, $originalPoster, $contentType, $content){
				$newValue = 1;
				try{
				...
				try{
					$statement = $this->db->prepare('INSERT INTO GroupContent(groupId, groupContentNumber, permissionId, wrote, previousPoster, originalPoster, contentType, content)
					VALUES(:groupId, :groupContentNumber, :permissionId, :wrote, :previousPoster, :originalPoster, :contentType, :content);');
					$statement->bindValue(':groupId', $groupId, PDO::PARAM_INT);
					$statement->bindValue(':groupContentNumber', $newValue, PDO::PARAM_INT);
					$statement->bindValue(':permissionId', $permissionId, PDO::PARAM_INT);
					$statement->bindValue(':wrote', $memberId, PDO::PARAM_INT);
					$statement->bindValue(':previousPoster', $previousPoster, PDO::PARAM_INT);
					$statement->bindValue(':originalPoster', $originalPoster, PDO::PARAM_INT);
					$statement->bindValue(':contentType', $contentType, PDO::PARAM_STR);
					$statement->bindValue(':content', $content, PDO::PARAM_STR);
			...
			public function getGroupContentInfo($groupId, $groupContentNumber){
				try{
					$statement = $this->db->prepare('SELECT groupId, groupContentNumber, permissionId, wrote, previousPoster, originalPoster, contentType, content, timeStamp
					FROM GroupContent WHERE memberId = :memberId AND groupContentNumber = :groupContentNumber;');
					$statement->bindValue(':memberId', $memberId, PDO::PARAM_INT);
					$statement->bindValue(':groupContentNumber', $groupContentNumber, PDO::PARAM_INT);
					$statement->execute();
					if ($statement->rowCount() > 0){
